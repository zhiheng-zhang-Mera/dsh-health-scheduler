...
因此消费者可以看到 `thermal` 在这次答案里只占了三分之一，而不是五分之一 —— 也能在同一个字段里
看到其余维度**一点都没算**。

**值得内化的推论：** 重新归一化意味着压力是一个关于*有数据的*维度的陈述，被缩放到 0–100。如果一台
机器只有 `worker` 在上报，而这一个维度得了 100 分，那么输出就是 `restart_pressure: 100`、
`coverage: 0.15`。这不是 bug，也不是对缺失维度求平均 —— 它的意思是「我们唯一能测的东西已经顶格
了」，而 `coverage` 与 `coverage_15pct` 这条理由，正是告诉运维方证据面有多窄的东西。由此有两条
实用规则：

- **先读 `coverage`，再读 `pressure`。** 0.15 coverage 下的 100 与 0.9 coverage 下的 100 是
  两种完全不同的主张，只有后者是关于整台机器的。
- 若希望压力代表「整个模型」，办法是让更多维度拿到遥测，而不是提高那个在上报维度的 `weights`：
  权重会被重新归一化，所以抬高其中一个并不会让它在缺失维度面前占到更大比重。
