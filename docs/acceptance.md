...
| `tests/rolling.test.js` | `RollingStore`, `TrendAnalyzer` | Window statistics, retention floors, band timing, slope and R² gates, projections, formatting. |
...
state and reason vocabulary, plus the `drive()` loop).
...
| --- | --- | --- | --- |
| 1 | Contains no `taskkill` / reboot execution logic. | Not asserted by a test. Verified by inspection: the only `execFile` call is `runCommandProbe`, which runs a user-configured telemetry probe; there is no process-kill or reboot API anywhere in `src/`. | **Manual** |
| 2 | The restart plugin is fully replaceable. | Not asserted by a test. Verified structurally: `RestartAdapter` is a two-method interface, `isRestartAdapter` is a structural check on `ctx.healthScheduler`, and `applyHealthScheduler` accepts an injected `restart` adapter. `RecordingRestartAdapter` in the rig *is* a drop-in replacement, which is what every scenario test uses. | **Manual** (design-level) |
| 3 | Every provider can fail independently. | `records provider failures without stopping the loop` | Yes |
| 4 | A single provider going down does not affect overall operation. | Partly: `records provider failures without stopping the loop` asserts one failure among four ticks and that the tick count is unaffected. `keeps ticking through a restart adapter that throws, without a request storm` asserts the loop survives an adapter that throws. There is no test that kills a *provider* for the whole backoff window and asserts the other providers keep being sampled. | **Partial** |
| 5 | With no telemetry the state is `unknown`, not `healthy`. | `returns unknown, not zero, for a missing value`; `stays unknown for a missing value regardless of duration`; `returns null statistics for a metric with no data instead of zero`; `marks the state DEGRADED when no telemetry is available at all`; `omits absent metrics instead of inventing zero` | Yes |
...
| 11 | Scheduled maintenance uses a window, not a hard time point. | `is outside the window before it opens`; `is before_target inside the window but ahead of the target`; `is at_target exactly on the target instant`; `is deferred while the deferral budget lasts, then overdue`; `never allows a request when the configuration forbids one` | Yes |
| 12 | `throttle` is preferred over `restart`. | `a sustained heat soak raises thermal pressure and throttles`; `holds a restart request until the maintenance window opens` | Yes |
...
| 14 | System reboot is used only for the escalation path. | Partly: level 4 requires pressure ≥ its own `enter`, which is strictly above level 3's, and `gateRestart` adds `system_reboot_requires_restart_adapter`. But the reason string `escalation_from_repeated_app_restart_failure` is attached unconditionally — there is **no** escalation counter and no restart-outcome feedback, so the engine does not actually verify that an app restart failed first. | **Partial** |
...
| 16 | Every decision is explainable. | `produces one explainable reason per applied action`; `never emits an action above the pressure that justifies it` | Yes |
| 17 | No decision storm over a long run. | `keeps ticking through a restart adapter that throws, without a request storm` (30 minutes, 120 evaluations, bounded to ≤ 3 adapter calls). There is no 6 h / 12 h / 24 h long-run test. | **Partial** |
| 18 | Uninstalling the plugin does not affect DS-Hns. | Not asserted, and not assertable from inside this plugin. The relevant evidence is structural: `tryResolveConfig` means a bad user document cannot fail the boot, provider, adapter, safe-point and log failures are all contained, tool registration failures are caught per tool, and `ctx.effect` stops the timer on dispose. It is containment, not a proof — `upstream.registerProvider` would propagate if a provider id were somehow duplicated. | **Manual** |
...
| `cpu_usage` measured from `os.cpus()` | No — the rig scripts it | Yes. Whether the delta is sensible on a given machine is a real-machine question. |
...
| A configured helper command actually producing parsable lines | No | Yes. `runCommandProbe` is exercised only through the environment seam in tests. |
| The stats file being written by a real integration | No | Yes. |
...
Stated plainly, so a maintainer can decide what to close first:

1. **No Long-Run tier.** Criteria 17 is verified for 30 minutes of scenario time, not 6 or 12
...
2. **No test that a provider's backoff window is honoured.** The circuit breaker's
   `disabledUntil`, `skipped` list and exponential schedule are unverified. The rig's
   `failNext(n)` plus a clock jump would cover it.
3. **No test of `normalizeSample`'s hard-bound snapping.** The `1e-9` relative tolerance path
   (a value just inside the bound is snapped rather than rejected) is not asserted.
4. **No test of the audit log at all.** `DecisionLog` rotation, `readPersisted`, the
   half-written trailing line and the write-failure path have no coverage.
5. **No test of `registerTools`, `renderHealthReport` or `metricsSnapshot`.** The model-facing
   surface and the report renderer are untested; a golden-output test on a fixed snapshot would
   be cheap.
6. **No test of `resolveConfig`.** The validation rules are documented and exercised indirectly
   (every test calls `resolveConfig`), but the rejection messages themselves are not asserted.
7. **No test of the settings namespace or `reconfigure`.** The live-reload path is unverified.
8. **No test that `apply` is defensive.** The entry point's containment is structural (config
   errors, provider failures, adapter failures, safe-point failures and per-tool registration
   failures are all caught), not asserted. A test that calls `applyHealthScheduler` with a
   minimal fake `ctx` and a malformed config would confirm it.
9. **Criterion 14's escalation reason is not backed by an escalation counter.** The reason
   `escalation_from_repeated_app_restart_failure` is emitted unconditionally with every level-4
